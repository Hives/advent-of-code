...
my $input-file = open "inputs/day_21.txt", :r;
...
sub run($program, $pointer-register, $reg-zero) {
    say "Zero register = $reg-zero";

    my $state = ($reg-zero, 0, 0, 0, 0, 0);
    ...
    loop {
        ...
        last TICK-LOOP if $state[$pointer-register] == 28
    }
...
sub part1(@input) {
    ...
    run($program, $pointer-register, 0)
}
