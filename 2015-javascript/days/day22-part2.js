...
function deductOneHitpointAndFilter({ gameHistories, minManaForWin }) {
    const newHistories = gameHistories.map(deductOneHitpoint);
    return removeCompleteGames({ gameHistories: newHistories, minManaForWin });
}

function deductOneHitpoint(history) {
    const lastRound = history.slice(-1)[0];
    const newRound = {
        ...lastRound,
        player: update({ hitpoints: -1 })(lastRound.player),
    };
    return history.concat(newRound)
}
